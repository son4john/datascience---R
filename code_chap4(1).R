...
C<-30    # coupon payment per six month that mean 2 payments for semi anual to convert to anual
...
simBV <- function(P,C,rvec,T) {#removed rvec
  BVvec = rep(0,T)
  for(t in 0:T) {
    i = t+1
    BVvec[i] <- BV(P,C,rvec[i],t,T)#AT EACH INTEREST RATE POINT SIMULATE THE BOND VALUE
  }
...
#added later simBV(P=1000,C=.06,T=10,r=.06,freq=1)
...
#Simulate 45 day long stock postion using rnorm
par(mfrow=c(1,1))
#Simulate rates market for r
#S(0)=$10000
T <- 45 # trading days 
Svec <- round(c(1,1+1.1*
              rnorm(T)*.0025),4)
SVvec <- 10000*Svec #intial value of 1000
